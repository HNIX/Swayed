class Api::V1::SourcesController < Api::InboundBaseController
  ...
  def ping
    ...
    if request.content_type == 'application/json'
        ping_lead_params = JSON.parse(request.body.read)
    else
        ping_lead_params = params[:lead]
    end
...
private 
  ...
  def authenticate_source
    token = request.headers['Authorization'] || params[:api_token]
    @source = Source.find_by(api_token: token)
    head :unauthorized unless @source
  end
