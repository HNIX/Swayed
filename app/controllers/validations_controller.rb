class ValidationsController < ApplicationController
    ...
    def set_validation_types
        case @field.data_type
            when 'date'
                @validation_types = Validation.validation_types.slice(:presence, :date)
            when 'text'
                @validation_types = Validation.validation_types.slice(:presence, :format, :value_length, :text_value)
            when 'number'
                @validation_types = Validation.validation_types.slice(:presence, :number_value)
            when 'boolean'
                @validation_types = Validation.validation_types.slice(:presence, :value)
            else
